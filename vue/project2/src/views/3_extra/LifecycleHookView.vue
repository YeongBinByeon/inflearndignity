...
export default {
  ...
  setup() {},
  // 실무에서는 before 붙은거는 거의 사용하지 않는다고 함
  // 인스턴스가 초기화되면 props 설정을 처리한 후 data() 또는 computed 같은 다른 옵션을 처리하기 직전에 호출이 됨
  beforeCreate() {},
  ...
  // 현재 컴포넌트를 빠져 나갈 때
  unmounted() {
    window.clearInterval(this.fnInterval) // interval 해제 처리
    this.fnInterval = null
    this.cities = null // 가비지컬렉션이 null이면 빨리 인식해서 메모리 해제함
  },
  methods: {
    getCityList() {
      // 서버에 도시 목록을 요청했다고 침
